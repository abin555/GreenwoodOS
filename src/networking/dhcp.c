...
static void dhcp_process_offer(struct ethernet_driver *driver, struct dhcp_packet *packet) {
    print_serial("Received DHCP offer from %d.%d.%d.%d: %d.%d.%d.%d\n", packet->siaddr[0], packet->siaddr[1], packet->siaddr[2], packet->siaddr[3], packet->yiaddr[0], packet->yiaddr[1], packet->yiaddr[2], packet->yiaddr[3]);
    arp_send_request(driver, packet->yiaddr);

    if (offer_count >= 2) {
        print_serial("Received too many DHCP offers\n");
        return;
    }
...
static void dhcp_process_ack(struct ethernet_driver *driver, struct dhcp_packet *packet) {
    print_serial("Received DHCP acknowledge from %d.%d.%d.%d\n", packet->siaddr[0], packet->siaddr[1], packet->siaddr[2], packet->siaddr[3]);
    ...
    while (op[0] != DHCP_OPTION_END) {
        if (op - packet->options >= 312) {
            print_serial("Received DHCP packet with invalid options\n");
            return;
        }
...
void dhcp_send_discover(struct ethernet_driver *driver) {
    ...
    udp_send_packet(driver, (uint8_t[]){0, 0, 0, 0}, 68, (uint8_t[]){255, 255, 255, 255}, 67, packet, sizeof(struct dhcp_packet));

    //free(packet);
}

void dhcp_send_request(struct ethernet_driver *driver, uint8_t server_ip[4], uint8_t requested_ip[4]) {
    print_serial("dhcp_send_request\n");
    struct dhcp_packet *packet = malloc(sizeof(struct dhcp_packet));
    packet->op = DHCP_BOOT_OP_REQUEST;
    packet->htype = DHCP_HTYPE_ETHERNET;
    packet->hlen = 6;
