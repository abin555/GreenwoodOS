...
static uint16_t ipv4_calculate_checksum(struct ipv4_packet *packet) {
    uint32_t checksum = 0;

    uint16_t *data16 = (uint16_t *)packet;
    for (size_t i = 0; i < sizeof(struct ipv4_packet) / 2; i++) {
        checksum += data16[i];
    }
...
int ipv4_send_packet(struct ethernet_driver *driver, uint8_t source_ip[4], uint8_t destination_ip[4], enum IPProtocol protocol, void *protocol_packet, size_t protocol_size, void *data, size_t data_size){
    print_serial("[IP] ip_send_packet\n");
    ...
    if (destination_ip[0] == 255 && destination_ip[1] == 255 && destination_ip[2] == 255 && destination_ip[3] == 255) {
        ...
        print_serial("[IP] Failed to get MAC address for destination IP address %d.%d.%d.%d\n", destination_ip[0], destination_ip[1], destination_ip[2], destination_ip[3]);
        return 0;
    } else {
        // Send to gateway
        if (!arp_get_mac(driver, driver->ipv4.gateway, destination_mac, 100)) {
            print_serial("[IP] Failed to get MAC address for gateway IP address %d.%d.%d.%d\n", driver->ipv4.gateway[0], driver->ipv4.gateway[1], driver->ipv4.gateway[2], driver->ipv4.gateway[3]);
            return 0;
        }
    }

    struct ipv4_packet *packet = malloc(sizeof(struct ipv4_packet) + protocol_size + data_size);
    packet->version = 4;
    packet->ihl = 5;
    packet->dscp = 0;
    packet->ecn = 0;
    packet->total_length = htons(sizeof(struct ipv4_packet) + protocol_size + data_size);
    ...
    memcpy((uint8_t *)packet + sizeof(struct ipv4_packet), protocol_packet, protocol_size);
    memcpy((uint8_t *)packet + sizeof(struct ipv4_packet) + protocol_size, data, data_size);

    packet->header_checksum = ipv4_calculate_checksum(packet);

    ethernet_send_packet(driver, destination_mac, (uint8_t *) packet, sizeof(struct ipv4_packet) + protocol_size + data_size, ETHERNET_TYPE_IP);
...
void ipv4_receive_packet(struct ethernet_driver *driver, struct ipv4_packet *packet, void *data, size_t data_size){
    print_serial("[IP] ip_receive_packet 0x%x 0x%x %d\n", driver, data, data_size);

    if (packet->version != 4) {
        print_serial("[IP] Invalid IP version %d\n", packet->version);
        return;
    }
    ...
    if (checksum != ipv4_calculate_checksum(packet)) {
        print_serial("[IP] Invalid IP checksum\n");
        return;
    }
    ...
    switch (packet->protocol) {
        ...
        case IP_PROTOCOL_UDP:
            ...
            udp_receive_packet(driver, packet, data, data + sizeof(struct udp_packet));
            break;
