...
void init_program_memory(){
    ...
    for(uint8_t slot = 0; slot < 10; slot++){
        create_page_entry(0x05000000 + 0x400000*slot, 0x05000000 + 0x400000*slot, 0x83);
    }
    IRQ_RES;
...
void load_program(uint8_t program_slot, struct FS_Item_Entry* file){
            uint8_t *data_buf = (uint8_t*) (0x05000000 + 0x400000*program_slot);
...
void select_program(uint8_t program_slot){
    create_page_entry(0x05000000 + 0x400000*program_active_slot, 0x05000000 + 0x400000*program_active_slot, 0x83);
    create_page_entry(0x05000000 + 0x400000*program_slot, 0, 0x83);
