...
void read(struct FILE *file, void *dest, size_t size){
	int head = file->head;
	fcopy(file, dest, size);
	fseek(file, head+size);
}
...
gd_open_gif(const char *fname)
{
    ...
    if (memcmp(sigver, "GIF", 3) != 0) {
		print_serial("[GIF] Invalid Signature\n");
        goto fail;
    }
    /* Version */
    read(fd, sigver, 3);
    if (memcmp(sigver, "89a", 3) != 0) {
		print_serial("[GIF] Invalid Version\n");
        goto fail;
    }
    /* Width x Height */
    width  = read_num(fd);
    ...
    if (!gif){
		print_serial("[GIF] Unable to alloc gif structure\n");
		goto fail;
	}
    gif->fd = fd;
    gif->width  = width;
...
read_plain_text_ext(gd_GIF *gif)
{
    if (gif->plain_text) {
        ...
        uint8_t cw, ch, fg, bg;
        off_t sub_block;
        fseek(gif->fd, gif->fd->head + 1); /* block size = 12 */
        tx = read_num(gif->fd);
        ty = read_num(gif->fd);
        tw = read_num(gif->fd);
        th = read_num(gif->fd);
        ...
        read(gif->fd, &fg, 1);
        read(gif->fd, &bg, 1);
        sub_block = gif->fd->head;
        gif->plain_text(gif, tx, ty, tw, th, cw, ch, fg, bg);
        fseek(gif->fd, sub_block);
    } else {
        /* Discard plain text metadata. */
		fseek(gif->fd, gif->fd->head + 13);
    }
...
read_ext(gd_GIF *gif)
{
    ...
    case 0xFF:
        ...
        break;
    default:
        print_arg("unknown extension: %02X\n", label);
    }
}
...
new_table(int key_size)
{
    ...
    Table *table = alloc(sizeof(*table) + sizeof(Entry) * init_bulk);
    if (table) {
