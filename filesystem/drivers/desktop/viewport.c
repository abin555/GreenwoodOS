...
void viewport_init_sys(struct ViewportList *viewport_list, struct WINDOW *win){
...
void viewport_send_event(struct Viewport *viewport, VIEWPORT_EVENT_TYPE event){
    ...
    */
    ...
    if(viewport->owner_program_slot != -1){
        task_setRunningID(vp_task_id);
        select_program(viewport->owner_program_slot);
    }
    
    viewport->event_handler(viewport, event);
    
    task_setRunningID(current_task_id);
    int slot = task_get_slot(current_task_id);
    if(slot != -1){
        select_program(slot);
    }
    task_lock(0);
