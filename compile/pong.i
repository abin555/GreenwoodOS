...
void pong(){
  ...
  while(1){
    ...
    if(physicscounter % physdelay == 0){
      ...
      ballx+=ballvx;
      bally+=ballvy;
      if(ballx > (int) fb_width - 50 || ballx < 50){
        if(ballx < 50){
          if(bally > paddley-(100 / 2) && bally < paddley+(100 / 2)){
            ballvx *= -1;
            score--;
    ...
    for(int drawPaddle = 0; drawPaddle < 100; drawPaddle++){
      fb_setPixel(50, paddley-(100/2)+drawPaddle,0xFFFFFF);
    }
